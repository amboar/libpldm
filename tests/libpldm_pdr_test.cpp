...
TEST(PDRAccess, getPLDMEntityfromPDR)
{
    ...
    uint32_t handle = 0;
    EXPECT_EQ(pldm_pdr_add_fru_record_set_check(repo, 1, 10, 1, 0, 100, &handle, false), 0);
    ...
    std::array<uint8_t, 29> data{212, 1, 0,   0, 1,  11, 0, 0, 19, 0,
                                 ...
                                 0,   0, 0,   0, 1,  10, 0, 1, 6};
    handle = 0;
    EXPECT_EQ(pldm_pdr_add_check(repo, data.data(), data.size(), false, 1, &handle), 0);
    ...
    std::array<uint8_t, 27> data2{10, 1, 0, 0,   1, 4,   0, 0, 17,
                                  ...
                                  5,  0, 0, 0,   1, 10,  0, 1, 6};
    handle = 0;
    EXPECT_EQ(pldm_pdr_add_check(repo, data2.data(), data2.size(), false, 1, &handle), 0);
    entity = pldm_get_entity_from_record_handle(repo, handle);
    ...
    handle = 0;
    EXPECT_EQ(pldm_pdr_add_check(repo, data3.data(), data3.size(), false, 1, &handle), 0);
    entity = pldm_get_entity_from_record_handle(repo, handle);
    ...
    for (const auto& type : notSupportedtypes)
    {
        ...
        hdr->type = type;
        uint32_t handle = 0;
        EXPECT_EQ(pldm_pdr_add_check(repo, data4.data(), data4.size(), false, 1, &handle), 0);
        entity = pldm_get_entity_from_record_handle(repo, handle);
...
TEST(PDRUpdate, testAddFruRecordSet)
{
    ...
    uint32_t handle = 0;
    EXPECT_EQ(
        pldm_pdr_add_fru_record_set_check(repo, 1, 10, 1, 0, 100, &handle, false), 0);
    EXPECT_EQ(handle, 1u);
    EXPECT_EQ(pldm_pdr_get_record_count(repo), 1u);
    ...
    handle = 0;
    EXPECT_EQ(
        pldm_pdr_add_fru_record_set_check(repo, 2, 11, 2, 1, 101, &handle, false), 0);
    EXPECT_EQ(handle, 2u);
    EXPECT_EQ(pldm_pdr_get_record_count(repo), 2u);
    EXPECT_EQ(pldm_pdr_get_repo_size(repo),
...
TEST(PDRUpdate, tesFindtFruRecordSet)
{
    ...
    uint32_t first = 1;
    EXPECT_EQ(pldm_pdr_add_fru_record_set_check(repo, 1, 1, 1, 0, 100, &first, false),
              0);
    uint32_t second = 2;
    EXPECT_EQ(pldm_pdr_add_fru_record_set_check(repo, 1, 2, 1, 1, 100, &second, false),
              0);
    uint32_t third = 3;
    EXPECT_EQ(pldm_pdr_add_fru_record_set_check(repo, 1, 3, 1, 2, 100, &third, false),
              0);
